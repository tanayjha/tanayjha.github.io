...
However, after using some basic probability theory I was able to convince myself that this is indeed the case.

Let's look at some math to derive this:

Let p be the probability that no two people in a room have the same birthday. Also, let us assume there are 365 days in a year (let's leave out leap years to make calculations simpler).
Now, the probability that at least two people will have the same birthday will be (1 - p).

Now, let's try to see if the statement above, i.e. if there are 23 people in a room, then the probability of same birthday is > 50%, is true or not.

Let's try to calculate p first, i.e. all the 23 people have different birthdays.
Once we have p, we can calculate the same birthday case with (1 - p), where the statement claims that it is > 0.5. Hence, p has to be < 0.5.

In order to calculate p, let's try to assign birthdays to these 23 people one by one, ensuring there is no collision.
...
Hence, the probability that none of them have birthdays on the same day is a product of all these choices, i.e.:

**p = (365/365) * (364/365) * (363/365) * ... * (343/365)**

I don't know if there is an easy way to do this calculation by hand, but if you use a computer/calculator to do this, you will see that this indeed comes out to be 49.27%, which means (1 - p), i.e. the probability of at least two same birthdays, is 50.73%, hence proving the statement.

## Some CS

With the above mathematics we did, we were able to kind of define a pattern to calculate the probability of two people having the same birthday given a number of people. Once we are able to define a function like this, we can convert it into a computer program and allow the computer to do all this hard work, and we can sit back and enjoy the result. I did exactly this.
...
The code runs in two modes. The first is the math aspect of getting the probability of same birthday as the number of people in the room increases from 1 to 100. It plots a graph of the number of people in the room on the x-axis and the probability of same birthday on the y-axis. Here is the graph:

{{< figure src="/images/birthdayParadox/peopleVsProb.png"  >}}

I have also marked two points of interest on the graph. The first is the point where the probability crosses 50%, and the second is the point where it crosses 99%.
...
And now for the simulation, I tried simulating the probability of same birthday when 30 people are in a room. Here are the results of the simulation:
...
Hopefully, through this blogpost, you got a taste of how counter-intuitive probability can be and how well computer science can aid us in solving some of these probabilistic problems with the random library and graphs.