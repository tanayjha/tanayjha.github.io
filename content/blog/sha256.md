...
2. **Rotate Right** - Rotates `pos` bits in the original number. Very similar to shift, but instead of dropping things off the end, it wraps them to the beginning (in a circular fashion).
...
4. **σ1 (lowerSigma1)** - Another combination of rotations and shifts.
...
7. **Choice** - Given three binary numbers x, y, z, and depending on the bits of x, chooses the corresponding bit in y if the x bit is 1, or the corresponding bit in z if the x bit is 0.
...
8. **Majority** - The result contains the majority bit out of each corresponding bit of x, y, and z.
...
The above picture shows the various steps of the SHA-256 algorithm. This is the SHA-256 algorithm in code that I implemented:
...
1. Convert the original message (usually a string) into the binary format. (If the string is "abc", it gets converted to `011000010110001001100011`, where the first 8 bits are "a" (97 ASCII), the next 8 are "b" (98 ASCII), and the next 8 are "c" (99 ASCII).)
...
3. Now that the message length is a multiple of 512, we just break it into message blocks that are 512 bits in length each.
...
4. The next step is to process each message block one by one and create a message schedule out of each block, eventually compressing each of the message schedules.
...
6. Now comes the heart of the SHA-256 algorithm: the compression stage. Now that we have 64 words of 32-bit size, we need to merge them in some way. SHA-256 does that by merging them down to 8 32-bit hash values. These hash values are initialized as below:
...
And these hash values are the ones that store the final result after processing each message block. As shown in the block diagram above, the updated hash values after each step are used as an input in the next step. The square roots of the first 8 primes are the initial hash values with which this whole process starts.
...
7. This process of creating a message schedule and compressing it is repeated for each message block. The final hash values, generated after processing the last message block, give us the SHA-256 hash. We convert each hash value into its 8-character hexadecimal equivalent and concatenate all of them to generate the 64-character SHA-256 hash.
