...
Detecting the starting point of a loop in a linked list is a very popular problem. In fact, most of you reading this might already know the algorithm to solve this. However, understanding why that algorithm works is a separate challenge altogether.

I was in the same state and decided to find an explanation of why it works. I could not find convincing explanations through simple Google searches, and hence decided to write this blog.
...
The above diagram shows an example of a loop.

Now we know that the pointers are going to meet at some point in the loop.

The second step of the algorithm says to bring one of the pointers to the start of the list.
Now move both the pointers (one at the start and one at the meeting point in the loop) one step at a time. **The next time they meet will be the starting point of the loop.**
...
- `m` - the number of links a pointer has to travel to reach the starting point of the loop.
...
`m` is the distance travelled to reach the start of the loop. Let's say it makes `p` rounds of the loop (notice that `p` can be zero) and finally it travels `k` distance from the loop start to reach the meeting point.


**Distance travelled by the fast pointer to reach the meeting point = m + q \* l + k**

This is similar to the slow pointer except that the fast pointer makes `q` rounds of the loop. Since it travels at twice the speed, `q > p`.

Now we know that: distance travelled by fast pointer = 2 \* distance travelled by slow pointer.

So, **m + q \* l + k = 2 \* (m + p \* l + k)  =>  (m + k) = (q - 2 \* p) \* l**

What this means is that `(m + k)` is an integral multiple of the length of the loop, or in other words, `(m + k) % l = 0`. So if you travel a distance of `(m + k)` within the loop, you will end up back at the start of the loop.

Now we can consider the second part of the algorithm. We bring one pointer to the start of the list.
When this pointer takes `m` steps, it reaches the start of the loop.
...
Now we know that `(m + k)` is a multiple of `l`. That means the second pointer has reached the start of the loop, and so has the first pointer. This implies that both pointers will meet at the beginning of the loop after the second step of the algorithm. Hence the algorithm works.
